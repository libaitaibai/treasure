 ...
 class topspeedModule extends MainBaseModule{
     public function index(){
         ...
         $sql_count = "SELECT count(*)
                    ...
                    WHERE
                        is_effect = 1 AND
                        success_time = 0 and is_topspeed=1 and is_number_choose=0 and is_pk=0 and is_coupons=0 ";
         if($cate_item)
         {
         ...
         $sql = "SELECT id, name,is_topspeed, max_buy,min_buy,unit_price,current_buy, progress,(max_buy-current_buy) as surplus_buy, icon
                FROM ".DB_PREFIX."duobao_item
                WHERE is_effect = 1 AND success_time = 0 and is_topspeed=1 and is_number_choose=0 and is_pk=0 and is_coupons=0 ";
         if($cate_item)
         {
