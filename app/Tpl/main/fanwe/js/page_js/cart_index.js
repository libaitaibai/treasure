...
function del_carts(objs,id,callback)
{
	...
	if(len>3){
		...
		jsondata[id]=0;
		$(".cart-list-footer-total .txt-red").html(parseInt($(".cart-list-footer-total .txt-red").text())-parseInt($(objs).parents("tr").find(".txt-red").text())+"元");
	}
	$.ajax({
        url:main_url,
...
function recount_total(id,num,type)
{
	...
	jsondata[id].total_price = jsondata[id].number * parseFloat(jsondata[id].unit_price);
		...
		$.each(jsondata,function(i,row){
			...
			//$(".select-bar .cart_input[data-id='"+row.id+"']").parents(".select-bar_td").siblings(".select-bar_total").find(".txt-red").html(Math.round(parseFloat(row.total_price)*100)/100+"元");
		...
		//$(".cart-list-footer-total .txt-red").html(Math.round(total_price*100)/100+"元");
